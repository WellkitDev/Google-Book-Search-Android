...
class Books extends ChangeNotifier {
  ...
  Future<void> getSearchedBookData(
    ...
    var url = !_free
        ? 'https://www.googleapis.com/books/v1/volumes?q=${byTitleNotAuthor ? 'intitle' : 'inauthor'} : $searchString&maxResult=$_singleLoadBookCount&startIndex=$startIndex'
        : 'https://www.googleapis.com/books/v1/volumes?q=${byTitleNotAuthor ? 'intitle' : 'inauthor'} : $searchString&filter=free-ebooks&maxResult=$_singleLoadBookCount&startIndex=$startIndex';
    try {
      _reachedEnd = false;
      setLoading(true);
